...
wanggang kaishi
  用户请求来了先进行2的32次方进行hash取模 ，取模之后会对数据进行平均分配到master1/2/3/4上，老师的图上：key1归master4，key2归master1，key3归master2，key4归master3（这里应该怎么理解，因为是顺时针，这也不是顺时针啊？）。    hash值是0到2的32次方-1，将这个范围的值平均分到4个节点，每个key过来对2的32次方取模，得到的值一定在0到2的32次方-1这个范围。

  如果某个节点挂了，查看第二章图，key2不变， key3按照顺时针落到master3上。那么这个时候master3上面的数据就多了，会不会出现问题，这时候会对已经分片的区域再进行分片，原来是4片，现在分为0~11片，也就是12片数据，比原来大了三倍，那么扩充之后就是master1负责0~3，master2负责4~7 以此类推。

  如果添加一个节点，查看第三章图，会按照顺时针从master4分一部分key到master5上。
wanggang jieshu
